...
class RangerTest {
    ...
    void Ranger_NoConditions_PropertiesShouldBeSetCorrectly() {
        String shouldStats = "Character{Class=class Characters.Ranger, damage=1, name='John', level=1, Strength=1, Dexterity=7, Intelligence=1, dps=1}";
    ...
    void increaseLevel_RangerExists_RangerLevelShouldBeIncreasedCorrectly() {
        ...
        String shouldStats = "Character{Class=class Characters.Ranger, damage=6, name='John', level=2, Strength=2, Dexterity=12, Intelligence=2, dps=1}";
        String isStats = ranger.stats();
        Assertions.assertAll(
                () -> assertEquals(shouldStats, isStats)
        );
